...
| `2026-09-11-invoices-bucket` | **Applied — partially verified.** Operator-verified in the Storage dashboard 2026-09-12: the `invoices` bucket exists, the Policies column shows `3`, and the file size limit is 5 MB (matches the migration's `5242880`). **Not confirmed:** that those three are specifically insert/select/delete scoped to `authenticated` rather than some other combination — the Policies tab wasn't opened. Consistent with the migration, not proof of it. A bucket created manually earlier could show a different policy set. Not checkable with the anon key either: `storage.buckets` RLS makes a bucket that definitely exists (`tx-attachments`) return the same `NoSuchBucket` as an invented name, and listing returns `[]`. Settle it in the Policies tab or with the service-role key before relying on the delete policy in particular |
...
10. **Accountant page follow-ups** — (a) `migrations/2026-09-12-accountant-page.sql` was **run on 2026-09-12**, in order after the staff-roles migration and the RLS lockdown (it replaces the lockdown's blanket `invoice_ledger` policy). Not independently re-verified — over an anon probe its policy replacement looks identical to the lockdown's. (b) PDF export is a deliberate fast-follow — CSV only in this pass. (c) The `invoices` and `tx-attachments` storage buckets grant `authenticated`, so the accountant can read invoice/receipt PDFs and delete from `invoices`; scope those policies by role. Confirm the actual policy set on `invoices` first — only the policy *count* has been verified (see Migration status).
