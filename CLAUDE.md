...
10. **Accountant page follow-ups** — (a) `migrations/2026-09-12-accountant-page.sql` was **run on 2026-09-12**, in order after the staff-roles migration and the RLS lockdown (it replaces the lockdown's blanket `invoice_ledger` policy). Not independently re-verified — over an anon probe its policy replacement looks identical to the lockdown's. (b) PDF export is a deliberate fast-follow — CSV only in this pass. (c) ~~The `invoices` and `tx-attachments` storage buckets grant `authenticated`~~ — policy set confirmed 2026-09-13 via `pg_policies`, and `migrations/2026-09-13-storage-bucket-roles.sql` is **written and awaiting a manual run**. It scopes both buckets by role. One constraint it encodes, worth not undoing: the accountant must **keep** select on `tx-attachments`, because `downloadTaxPackage()` on the Expenses tab signs a URL for every receipt and `expenses` is in `ROLE_TABS.accountant` — revoking it fails silently, each receipt just dropping out of the ZIP as `failed`.
