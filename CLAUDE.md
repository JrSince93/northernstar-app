...
| `2026-09-18-participants-pending-invoice-column` | **WRITTEN, NOT RUN.** Adds `participants.pending_invoice jsonb` (nullable, no default) — the column the invoice-generation path has been writing at ~line 7228 and clearing at ~line 8429 since it was built, which never existed. Schema only: no code ships with it, because the call sites already assume the column. Nothing to backfill; null already means "none pending". Until it runs, both writes keep failing silently (fire-and-forget `.then`, error branch only skips the local-state update) and the readers at ~lines 7540 and 8278 see nothing, so a generated invoice's pending link is lost on reload. See Pending work item 12 |
...
12. **`participants.pending_invoice` does not exist** — migration **WRITTEN, NOT RUN** (`migrations/2026-09-18-participants-pending-invoice-column.sql`). The column was never created, but the app writes and reads it. Found 2026-09-13 while mapping the schema for the Next.js rebuild: an anon `?select=pending_invoice` probe returns `42703 column does not exist`, while every other column on `participants` (including `budget_lines`, `invoice_files`, `archived_at`, `roster`) returns `200`. Controls with invented column names were run alongside, so the check discriminates.
    ...
    **Two ways to close it**: add the column (`alter table public.participants add column if not exists pending_invoice jsonb;`) in a migration, which is what the code assumes; or drop the two writes and accept session-only pending state, in which case delete the readers too. Prefer the migration — the readers exist because the persistence was intended. **Taken 2026-09-18:** `migrations/2026-09-18-participants-pending-invoice-column.sql` adds the column, schema only, with the call sites deliberately untouched. It has **not been run**, so until it is, nothing changes — the writes keep failing silently and pending state stays session-only.
