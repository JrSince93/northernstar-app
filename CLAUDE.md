...
Migrations run manually in the SQL editor, and nothing records that they ran — which is how `2026-08-29-budget-line-linking.sql` sat unapplied for two weeks while every later migration went in (see below). **Update this table whenever you run one**, and check it before assuming a column exists.

Verified 2026-09-12 by probing the live schema as `anon` (`?select=<column>` returns `200 []` when a column exists, `400 / 42703` when it does not; controls with invented column names were run alongside to confirm the check discriminates).

### Check the ledger first, not this prose

`public._migrations_applied` (created by `migrations/2026-09-18-migration-ledger.sql`) is a queryable record of what has actually been run. **It is the authoritative answer to "is X already applied?"** — this table is hand-maintained prose and has drifted in both directions: on 2026-09-18 it still said `2026-09-13-storage-bucket-roles` was `WRITTEN, NOT RUN` while those policies were live in production, and `2026-08-29-budget-line-linking` sat unapplied for two weeks while every later migration went in.

- **Before assuming anything about a migration**, query the ledger: `select filename, applied_at, applied_by, notes from public._migrations_applied order by filename;`. Fall back to this table's prose, or to probing the live schema, only where the ledger has no row.
- **Immediately after running any migration in the SQL editor**, insert its row — before or alongside updating this table by hand. A run that isn't recorded is the whole problem:

  ```sql
  insert into public._migrations_applied (filename, applied_by, notes)
  values ('<YYYY-MM-DD-name>.sql', '<who ran it>', '<what was verified>');
  ```

- A file in `migrations/` with **no ledger row is unknown, not unapplied**. Verify against the live schema before running it, since not every migration is safe to re-run.
- The ledger records *that* a file ran; the table below records *what it changed and how it was verified*. Keep both — neither replaces the other.

| Migration | Status |
|---|---|
| `2026-08-29-budget-line-linking` | **Applied 2026-09-12** — see the gap note below |
| `2026-09-04-expense-tracker` | Applied (`transactions.allocated_to`/`.participant_id`, `recurring_expenses`, `recurring_expense_instances` all present) |
| `2026-09-05-dismissed-recurring-candidates` | Applied (table present) |
| `2026-09-05-recurring-multi-occurrence` | Applied — **columns only verified**; the unique-constraint swap to `(recurring_expense_id, month, occurrence_number)` is not visible over PostgREST. If multi-occurrence inserts ever fail on the old 2-column constraint, that's the part that didn't take |
| `2026-09-05-recurring-weekday-cadence` | Applied (`cadence_weekday`) |
| `2026-09-07-recurring-interval-months` | Applied (`interval_months`, `tracked_since`) |
| `2026-09-07-transaction-notes-attachments` | Applied (`note`, `attachment_path`) |
| `2026-09-09-participant-archive` | Applied (`participants.archived_at`) |
| `2026-09-11-invoices-bucket` | **Applied — confirmed 2026-09-13.** The bucket exists with a 5 MB limit (matches the migration's `5242880`), and a `pg_policies` query on `storage.objects` shows exactly the three policies it creates: `invoices select`/`insert`/`delete`, each `to authenticated` with a `bucket_id = 'invoices'` check. (Note: bucket existence is *not* checkable with the anon key — `storage.buckets` RLS makes a bucket that definitely exists return the same `NoSuchBucket` as an invented name, and listing returns `[]`. Use `pg_policies` in the SQL editor instead.) Superseded by `2026-09-13-storage-bucket-roles` once that runs |
| `2026-09-11-staff-roles` | Applied 2026-09-12 (operator-run). Independently confirmed: `public.staff` went from `404 PGRST205` to `200` |
...
| `2026-09-13-storage-bucket-roles` | **APPLIED — verified 2026-09-18, but NOT run by this session.** The full policy set was already in place when checked, and nothing records when it was executed (the project has no `supabase_migrations` history table, so the table you are reading is the only record). **Do not re-run the file as-is:** its `drop policy if exists` statements name the *old* policy names, which are gone, while `create policy` has no `if not exists` — it would raise `42710 duplicate_object` and roll the transaction back. Verified via `pg_policies` on `storage.objects`: 12 policies — the 7 anon onboarding ones untouched, plus exactly the 5 this migration creates, every one gated on `current_staff_role()`. `invoices`: select for admin/office_manager/accountant; insert and delete for admin/office_manager only. `tx-attachments`: select for all three roles (the accountant read `downloadTaxPackage()` depends on — intact); insert for admin/office_manager only. No UPDATE policy on either bucket and no DELETE on `tx-attachments`, as intended. Live checks: the real admin resolves to `admin`, sees 2 invoice objects and 12 receipts, and passes the select/insert/delete predicates; a signed-in user with no `staff` row gets `current_staff_role() = null` and sees 0 objects in both buckets, so it fails closed. Accountant and office_manager were **not** tested live — `staff` and `auth.users` each hold exactly one row (the admin), so simulating either would mean mutating the sole admin's role |
| `2026-09-14-lock-onboarding-tables` | **APPLIED — verified 2026-09-18, true apply date unknown.** Added to this table 2026-09-18: the file (commit `ab65453`) had no row here at all, so its state had to be read from the database. Live state matches the file statement for statement — RLS on both tables, and exactly the two policies it creates: `anon can submit` (INSERT only, no USING clause) and `authenticated full access` (ALL, using `true`). A live read as the `anon` role returns **0 rows** from both `onboarding_submissions` and `doc_signatures`, so the exposure it was written to close is closed. **Caveat:** anon still holds the table-level SELECT *grant* on both — this file never revokes grants, and that grant is the Supabase default on every `public` table — so protection rests entirely on RLS plus the absence of an anon SELECT policy. Adding one, or disabling RLS, re-exposes 18 rows of names, DOBs, addresses, TFN/ABN and bank details. Still open from the file's own notes: the OAIC notifiable-data-breach assessment for the already-exposed records, and replacing anon read-back with a SECURITY DEFINER token function |
| `2026-09-15-ndis-2026-27-non-sil-rates` | **RUN 2026-09-18.** Data-only (no schema change): rewrites the stored `budget_lines[].rates` on `core`, `community`, `core_combined` and `employment` lines to 2026-27 prices, and corrects community's Saturday/Sunday/travel codes. A row changes only while it still holds its exact 2025-26 default (the `migrateSilRates2026` guard), and core lines without a sleepover row get a `01_010_0107_1_1` one. Touches `rates` only, includes archived participants, idempotent. A read-only dry run on 2026-09-15 matched 15 rate rows on 2 lines: Abdi's `bl-legacy` (core) and Lita's `bl-1783273601383` (core_combined). The run updated both lines; re-running the dry run afterwards reported 0 remaining changes. **Invoices issued before it — INV-CF-20260719, -0816 and -0830 (paid) and INV-CF-20260913 (issued to Lita's plan manager, outstanding) — bill 2025-26 rates and are historical record: not corrected, not reissued.** Reload the app after running this: the participant modal writes its on-screen rate table back on save |
| `2026-09-18-schedule-block-split-times` | **RUN 2026-09-18.** Adds nullable `flat_start`, `flat_end`, `active_start`, `active_end` (type `time`, matching `start_time`/`end_time`) to `schedule_blocks`, so a sleepover block's entered flat/active windows persist instead of being re-inferred from `start_time`/`end_time` + `flat_hours`/`active_hours` every time the form reopens. Deliberately no backfill: the old rows' real windows were never recorded, so they keep null and stay on the inference path. Verified after the run: all four exist as nullable `time without time zone`, matching `start_time`/`end_time`. Pre-existing rows are null and stay on the inference path until each block is next saved, and `upsertScheduleBlockWithColumnFallback`'s strip-and-retry path (with its "DB is missing flat/active split columns" toast) is now dormant rather than load-bearing |
| `2026-09-18-participants-pending-invoice-column` | **RUN 2026-09-18.** Adds `participants.pending_invoice jsonb` (nullable, no default) — the column the invoice-generation path has been writing at ~line 7228 and clearing at ~line 8429 since it was built, which never existed. Schema only: no code ships with it, because the call sites already assume the column. Nothing to backfill; null already means "none pending". Verified after the run: nullable `jsonb`, comment applied, both participant rows null as expected. The writes at ~7228 and ~8429 now land instead of failing silently, so a generated invoice's pending link survives a reload. See Pending work item 12 |

The operator also confirmed the production admin login still works after all three auth migrations.

### The budget-line-linking gap (resolved)

`2026-08-29-budget-line-linking.sql` was written on 29 Aug but **not run until 12 Sep**, though every migration dated after it had been applied. Found while auditing the `migrations/` directory against the live schema: `invoice_ledger.budget_line_id`, `invoice_ledger.rate_card` and `transactions.budget_line_id` all returned `42703 column does not exist`. Re-checked after the operator ran it — all three now return `200`, and a combined `select=budget_line_id,rate_card` also succeeds.

Nothing was broken by the delay, which is why it went unnoticed: the columns are nullable and `attributeReceiptsToLines()` falls back to string-matching `transactions.reference` against `BL_INV_PREFIXES`. The cost was the failure mode the migration exists to remove — one mistyped or truncated `invoice_ref` silently sends a payment to "Unattributed" on the Reports tab, with no structural link from invoice generation → ledger → cash book.

Two things follow from it:
- Historical rows still have null `budget_line_id`, so the prefix-matching fallback stays load-bearing for everything generated before 12 Sep. Don't delete it on the assumption the columns are now always populated.
- The lesson is the missing ledger, not this one file. A migration whose absence degrades gracefully can stay unapplied indefinitely without any symptom — hence the table above.

## Pending work

1. ~~**Staff roles, Phase 1**~~ — `migrations/2026-09-11-staff-roles.sql` was **run on 2026-09-12**. `staff` exists, and the "Allow all" policies on `pay_runs`, `employees`, `participants` and `transactions` are replaced with role policies. Remaining: scope `dropdown_options` by role, which the roles migration deliberately left alone.
2. ~~**RLS lockdown (Auth Phase 3)**~~ — `migrations/2026-09-11-rls-lockdown.sql` was **run on 2026-09-12**. Verified: `transactions`, `employees`, `participants`, `pay_runs` and `invoice_ledger` all returned real rows to the bare anon key before it ran, and return `[]` after. **This closed a live data exposure** — the anon key is embedded in `public/index.html` on a publicly reachable site, so until 12 Sep anyone with it could read employee contact details, participant NDIS numbers, DOBs and addresses, every pay run and the whole invoice ledger without authenticating.
...
10. **Accountant page follow-ups** — (a) `migrations/2026-09-12-accountant-page.sql` was **run on 2026-09-12**, in order after the staff-roles migration and the RLS lockdown (it replaces the lockdown's blanket `invoice_ledger` policy). Not independently re-verified — over an anon probe its policy replacement looks identical to the lockdown's. (b) PDF export is a deliberate fast-follow — CSV only in this pass. (c) ~~The `invoices` and `tx-attachments` storage buckets grant `authenticated`~~ — policy set confirmed 2026-09-13 via `pg_policies`, and `migrations/2026-09-13-storage-bucket-roles.sql` is **applied — confirmed 2026-09-18** (see the migration status table; it was already in place, and re-running the file verbatim would error). It scopes both buckets by role. One constraint it encodes, worth not undoing: the accountant must **keep** select on `tx-attachments`, because `downloadTaxPackage()` on the Expenses tab signs a URL for every receipt and `expenses` is in `ROLE_TABS.accountant` — revoking it fails silently, each receipt just dropping out of the ZIP as `failed`.
...
12. ~~**`participants.pending_invoice` does not exist**~~ — **RESOLVED 2026-09-18**, `migrations/2026-09-18-participants-pending-invoice-column.sql` run against production; the column now exists as nullable `jsonb`. The history is kept because the failure mode is the point. The column was never created, but the app wrote and read it. Found 2026-09-13 while mapping the schema for the Next.js rebuild: an anon `?select=pending_invoice` probe returns `42703 column does not exist`, while every other column on `participants` (including `budget_lines`, `invoice_files`, `archived_at`, `roster`) returns `200`. Controls with invented column names were run alongside, so the check discriminates.

    The write is `sb2.from('participants').update({pending_invoice:pendingInv})` (~line 7228, in the invoice-generation path) and the clear is at ~line 8429. Both are fire-and-forget `.then(...)` calls whose error branch only skips the local-state update — nothing surfaces, so this has been failing silently since it was written.

    **Impact is limited but real.** The in-memory `pt.pending_invoice` is still set by the same code paths, and `rememberPendingGenInvoice` / `_lastGenInvoice` keep the session's generated invoices, so attaching an invoice works within a session. What is lost is the *persistence* the column exists for: generate an invoice, reload before attaching it, and the pending link is gone — which is the exact failure the "keep EVERY invoice generated this session" comment at ~line 8253 was written to fix, only across reloads rather than within one. The readers at ~lines 7540 and 8278 just see nothing.

    **Two ways to close it**: add the column (`alter table public.participants add column if not exists pending_invoice jsonb;`) in a migration, which is what the code assumes; or drop the two writes and accept session-only pending state, in which case delete the readers too. Prefer the migration — the readers exist because the persistence was intended. **Done 2026-09-18:** `migrations/2026-09-18-participants-pending-invoice-column.sql` added the column, schema only, with the call sites deliberately untouched, and it has been run against production — so pending state now persists across a reload. The remaining follow-up is the last line below.

    Worth checking whether any *other* column the app writes is in the same state: this one survived because every write to it is fire-and-forget.
